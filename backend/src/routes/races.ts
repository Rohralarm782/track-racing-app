  ...
  try {
    ...
    if (race.categoryId) {
      ...
      await prisma.$transaction([
        ...
        // Punkte mitnehmen, falls im Dokument vorhanden (z.B. Omnium-Zwischenstand)
        ...toInclude
          .filter(t => byNumber.get(t.number)?.points != null)
          .map(t => prisma.omniumScore.upsert({
            where: { raceId_teamId: { raceId: race.id, teamId: t.id } },
            create: { raceId: race.id, teamId: t.id, points: byNumber.get(t.number)!.points! },
            update: { points: byNumber.get(t.number)!.points! },
          })),
      ]);
      ...
      res.json({
        ...
        pointsImported: toInclude.filter(t => byNumber.get(t.number)?.points != null).length,
      });
      return;
    }
    ...
    // Punkte mitnehmen, falls im Dokument vorhanden (z.B. Omnium-Zwischenstand vorm Punktefahren)
    const byNumber = new Map(teams.map(t => [t.number, t]));
    const withPoints = upserted.filter(t => byNumber.get(t.number)?.points != null);
    if (withPoints.length > 0) {
      await prisma.$transaction(
        withPoints.map(t => prisma.omniumScore.upsert({
          where: { raceId_teamId: { raceId: race.id, teamId: t.id } },
          create: { raceId: race.id, teamId: t.id, points: byNumber.get(t.number)!.points! },
          update: { points: byNumber.get(t.number)!.points! },
        }))
    ...
    res.json({
      mode: 'direct', created: upserted.length, removed: toRemove.length,
      pointsImported: withPoints.length,
    });
  } catch (e) { next(e); }
});
