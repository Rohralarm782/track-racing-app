...
- lauf / laufLabel: beide beziehen sich AUSSCHLIESSLICH auf eine echte Lauf-Spalte der Tabelle (Spaltenüberschrift "Lauf", "Heat", "Paarung" o.ä.). Gibt es keine solche Spalte, sind BEIDE null. Die Startnummer ("Start-Nr.") ist NIEMALS die Lauf-Nummer — verwechsle die beiden Spalten nicht. Auch eine Überschrift wie "Vorlauf 1" über der Tabelle ist KEINE Lauf-Angabe im Sinne dieser Felder: dann beide null.
  ...
  * lauf: der Wert der Lauf-Spalte, wenn er eine Zahl enthält. Steht dort NUR eine Zahl (z.B. "9"), ist lauf=9. Steht direkt bei der Zahl zusätzlich ein Buchstaben-Code — "Z 9", "G 9", "ZG 9", "GG 9" —, dann in EINEM Schritt aus GENAU DERSELBEN Zelle BEIDES herauslesen: lauf=9 UND (siehe startPos unten) startPos aus genau diesem Buchstaben-Code ("Z"/"ZG" -> "ZG", "G"/"GG" -> "GG"). Den Buchstaben-Code NICHT als Rauschen verwerfen, nachdem die Zahl entnommen ist — er ist in solchen Tabellen die einzige Quelle für startPos, siehe Beispiel dort.
  * laufLabel: der Text der Lauf-Spalte, wenn er KEINE Zahl enthält — z.B. bei Sprint-Finals steht dort "Platz 1/2" bzw. "Platz 3/4", bei Hoffnungsläufen o.ä. auch anderer Text. Wortlaut aus dem Dokument übernehmen (Zeilenumbrüche in der Zelle als Leerzeichen). Enthält die Zelle eine Zahl (mit oder ohne Buchstaben-Code davor), dann laufLabel null.
...
- startPos: die Startposition dieses Fahrers/Teams. Genau einer dieser vier Werte oder null:
  * Einzelstart-Formate (Zeitfahren, Einzel-/Mannschaftsverfolgung — je nach Format EIN oder ZWEI Starter pro Lauf; beim 1000m-Zeitfahren startet oft nur eine Fahrerin pro Lauf, das ist normal und macht die Lauf-Spalte nicht ungültig): "ZG" (Zielgerade) oder "GG" (Gegengerade). Zwei mögliche Quellen dafür, unabhängig voneinander zu prüfen:
    (a) SPALTENWERT SELBST — Beispiel: zwei Zeilen "Z 5" und "G 5" (oder "ZG 5"/"GG 5") für denselben Lauf 5, je eine pro Fahrer. Das ist der Normalfall bei dieser Kombination aus Lauf-Spalte und Einzelstart-Format. Bereits bei lauf oben beschrieben: BEIDE Werte (lauf UND startPos) kommen aus GENAU DERSELBEN Zelle, in einem Schritt, nicht in zwei getrennten Durchgängen. "Z"/"ZG" -> "ZG", "G"/"GG" -> "GG".
