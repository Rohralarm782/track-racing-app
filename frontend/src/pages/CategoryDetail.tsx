...
const STATUS_BADGE: Record<string, string> = {
  SETUP: 'badge-gray', ACTIVE: 'badge-yellow', FINISHED: 'badge-green',
};
const STATUS_LABEL: Record<string, string> = {
  SETUP: 'Vorbereitung', ACTIVE: 'Laeuft', FINISHED: 'Fertig',
};
...
export default function CategoryDetail() {
  ...
  async function createRace() {
    ...
    try {
      ...
      setRaceError(e.message ?? 'Fehler');
      setSavingRace(false);
    }
  }

  if (loading) return (
    <div className="page container"><div className="loading"><span className="spinner" /> Laedt...</div></div>
  );
  if (!category) return (
    ...
    <div className="page container">
      ...
      {showImport && (
        <div className="card mb-4">
          ...
          <TeamBulkEntry
            categoryId={category.id} format={category.format}
            existingTeams={teams}
        ...
        <>
            ...
            <div className="card mb-3" style={{ borderColor: '#bfdbfe', background: '#f0f7ff' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div className="form-group" style={{ margin: 0 }}>
                  ...
                  <select className="form-select" value={raceType}
              ...
              <div className="flex-between mt-3">
                <button className="btn btn-ghost btn-sm"
                  onClick={() => { setShowNewRace(false); setRaceError(''); }}>Abbrechen</button>
                <button className="btn btn-primary" onClick={createRace}
                  disabled={savingRace || !raceName}>
          ...
          {races.length === 0 ? (
            <div className="empty">
              ...
              {isAdmin && teams.length === 0 && (
                <p className="text-sm">Zuerst die Startliste einpflegen.</p>
              )}
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
              {races.map(race => (
                <Link key={race.id} to={`/races/${race.id}`}
                  className="card card-link" style={{ display: 'block' }}>
                  <div className="flex-between">
                    ...
                    <span className="badge badge-gray" style={{ fontSize: 11 }}>
                      {race.type === 'PUNKTEFAHREN' ? 'Punktefahren'
                        : race.type === 'TEMPORUNDEN' ? 'Temporunden'
                        : 'Verfolgung'}
                    </span>
                  </div>
                </Link>
